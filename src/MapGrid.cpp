...
void MapGrid::sawOffCorners() {
    ...
    for (int j = 1; j < n_cells_.y - 1; ++j) {
        for (int i = 1; i < n_cells_.x - 1; ++i) {
            const auto cell_index = j * n_cells_.x + i;
            if (tiles2[cell_index].isSquare()) {
                wall_rect.setPosition(i * cell_size_.x, j * cell_size_.y);
                walls_drawable_.push_back(wall_rect);
            } else {
                switch (tiles2[cell_index].type) {
                case WallType::DLTRIANGLE:
                    dl_triangle.setPosition(i * cell_size_.x, j * cell_size_.y);
                    walls_drawable_.push_back(dl_triangle);
                    break;
                case WallType::ULTRIANGLE:
                    ul_triangle.setPosition(i * cell_size_.x, j * cell_size_.y);
                    walls_drawable_.push_back(ul_triangle);
                    break;
                case WallType::DRTRIANGLE:
                    dr_triangle.setPosition(i * cell_size_.x, j * cell_size_.y);
                    walls_drawable_.push_back(dr_triangle);
                    break;
                case WallType::URTRIANGLE:
                    ur_triangle.setPosition(i * cell_size_.x, j * cell_size_.y);
                    walls_drawable_.push_back(ur_triangle);
                    break;
                }
            }
        }
...
void MapGrid::extractEdgesFromTiles(Triangulation& cdt) {
    ...
    for (int j = 1; j < n_cells_.y - 1; ++j) { //! look for horizontal edges
        for (int i = 0; i < n_cells_.x; ++i) {
            ...
            if (!visited[cell_index] and isBoundaryWallTile(cell_index, Direction::UP)) {
                ...
                do {
                    ...
                    if (direction % 2 == 1) { //! the direction is diagonal thus the edge is longer
                        e.l *= std::sqrt(2.f);
                    }
...
void MapGrid::extractVerticesForDrawing(Triangulation& cdt, std::vector<TriInd>& tri_ind2component) {
    ...
    int tri_ind = 0;
    for (int j = 1; j < n_cells_.y - 1; ++j) { //! Scanning left-right
        for (int i = 1; i < n_cells_.x - 1; ++i) {
            const auto cell_index = j * n_cells_.x + i;
    ...
    //     const auto r_center = cdt.calcTriangleCenter(tri);

    //     if (tri_ind2component[tri_ind] == 3) {

    //         walls_vertices_.append({asFloat(tri.verts[0]), component_color});
    //         walls_vertices_.append({asFloat(tri.verts[1]), component_color});
    //         walls_vertices_.append({asFloat(tri.verts[2]), component_color});
    //     }
    //     tri_ind++;
    // }
    walls_drawable_.clear();
...
void MapGrid::extractEdgesFromTilesV2(Triangulation& cdt) {
    ...
    for (int j = 1; j < m - 1; ++j) { //! Scanning left-right
        for (int i = 1; i < n - 1; ++i) {
            const auto cell_index = j * n_cells_.x + i;
...
Building MapGrid::addBuildingEdgesToTriangulation(sf::Vector2f building_center, sf::Vector2i building_size,
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        if (i % 2 == 0) {
            std::sort(edges_on_line2.begin(), edges_on_line2.end(),
                      [](const Edgef& e1, const Edgef& e2) { return std::min({e1.from.x, e1.to().x}) < std::min({e2.from.x, e2.to().x}); });
        } else {
            std::sort(edges_on_line2.begin(), edges_on_line2.end(),
                      [](const Edgef& e1, const Edgef& e2) { return e1.from.y < e2.from.y; });
        }
    }
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        std::sort(edges_on_line2.begin(), edges_on_line2.end(),
                  [](const Edgef& e1, const Edgef& e2) { return e1.from.x < e2.from.x; });
    }
