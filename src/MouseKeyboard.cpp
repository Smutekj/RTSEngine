...
{
    return {-1, -1} ;//priv::InputImpl::getMousePosition();
}
...
Vector2i Mouse::getPosition(const Window& relativeTo)
{
    ...
    glfwGetCursorPos(relativeTo.handle, &pos.x, &pos.y);
    int winWidth, winHeight;
    glfwGetWindowSize(relativeTo.handle, &winWidth, &winHeight);
    int fbWidth, fbHeight;
    glfwGetFramebufferSize(relativeTo.handle, &fbWidth, &fbHeight);
